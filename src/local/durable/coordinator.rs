...
pub struct DurableCoordinator {
    ...
    // One group-commit lane per journal shard, indexed like the shard files,
    // so fsync streams for different shards proceed in parallel.
    block_journal_flush: Arc<Vec<BlockJournalFlushCoordinator>>,
...
struct BlockJournalMaterializationPauseState {
    ...
    pause_in_sync_stage: bool,
    paused_in_sync_stage: bool,
    resume_sync_stage: bool,
...
impl DurableCoordinator {
    ...
    fn open_with_policies(
        ...
        let block_journal_flush: Arc<Vec<BlockJournalFlushCoordinator>> = Arc::new(
            (0..durable.block_journal_shard_count())
                .map(|_| BlockJournalFlushCoordinator::new())
                .collect(),
        );
    ...
    #[cfg(test)]
    fn fail_next_block_journal_sync_for_test(&self) {
        self.durable
            .fail_next_block_journal_sync
            .store(true, Ordering::SeqCst);
    }

    #[cfg(test)]
    fn delay_next_block_journal_sync_for_test(&self, delay: Duration) -> Result<()> {
        *lock(&self.durable.block_journal_sync_delay)? = Some(delay);
        Ok(())
    }

    #[cfg(test)]
    fn fail_next_block_journal_tail_recovery_for_test(&self) {
        self.durable
    ...
    fn record_block_journal_profile(
        ...
        lane_timing: BlockJournalLaneBatchTiming,
    ) -> Result<()> {
        self.record_persist_profile(DurablePersistProfile {
            ...
            block_journal_payload_recheck_nanos: lane_timing.payload_recheck_nanos,
            block_journal_publish_nanos: lane_timing.publish_nanos,
            ...
            block_journal_pipeline_order_wait_nanos: lane_timing.pipeline_order_wait_nanos,
            durable_commit_high_water: durable_commit_high_water.raw(),
            ..DurablePersistProfile::default()
        })
    }

    /// Journal shard lane for one device's records.
    ///
    /// Routing is a stable function of the device id and the persisted shard
    /// count, so every record for a device lives in one shard file and waits
    /// on that shard's lane.
    fn block_journal_lane(
        &self,
        device_id: DeviceId,
    ) -> Result<(usize, &BlockJournalFlushCoordinator)> {
        let shard = self.durable.block_journal_shard_for_device(device_id);
        let lane = self
            .block_journal_flush
            .get(shard)
    ...
    /// Hand a request to its device's lane and return the id to wait on.
    ///
    /// Since M7 this only POSTS the request (see
    /// `BlockJournalFlushCoordinator::post_arrival`): it takes no lane
    /// mutex and issues no wakeup, so the write paths — which all run this
    /// inside the global staging hold — no longer serialize the whole store
    /// behind one writer's lane-mutex acquisition and `notify_all`. The
    /// arrival is folded into `pending` (and the lane woken) by the first
    /// thread to take the lane state afterwards, which for every caller is
    /// its own immediately-following `wait_for_block_journal_request`.
    fn enqueue_block_journal_request(
        &self,
        ...
        let (shard, lane) = self.block_journal_lane(device_id)?;
        let request_id = lane.post_arrival(request)?;
        Ok((shard, request_id))
    }

    /// Wait until one shard's group-committed journal lane completes a request.
    ///
    /// The first waiter that finds the WRITE stage free becomes the batch
    /// owner: it coalesces nearby requests per the batch policy, appends
    /// them as one journal write, then hands the write stage to the next
    /// batch and drives its own data sync and publish through the ticketed
    /// SYNC stage — so batch N+1 encodes and writes while batch N's fsync
    /// is in flight. Other waiters block until their request completes, so
    /// every waiter still observes its own durability boundary. Lanes on
    /// different shards run independently.
    fn wait_for_block_journal_request(&self, shard: usize, request_id: u64) -> Result<()> {
        let lane = self
            .block_journal_flush
            .get(shard)
            .ok_or_else(|| StorageError::corrupt("block journal shard has no lane"))?;
        loop {
            // This acquisition is also what folds this caller's own posted
            // arrival into `pending` and wakes the lane (M7).
            ...
            if !state.write_stage_busy && state.admits_election() && !state.pending.is_empty() {
                ...
                let (batch, ticket) = match self.coalesce_block_journal_requests(lane, state) {
                    Ok(mut state) => {
                ...
                let _ = self.persist_block_journal_lane_batch(
                    Instant::now(),
                    shard,
                    lane,
                    ticket,
            ...
            let generation = state.generation;
            while state.generation == generation && !state.completed.contains_key(&request_id) {
                state = lane.wait_arrivals(state)?;
            }
        }
    }
    ...
    fn wait_for_block_journal_lanes_idle(&self) -> Result<()> {
        for shard in 0..self.block_journal_flush.len() {
            ...
            // it can post to this lane — so for as long as this thread is
            // here, no write can arrive on this lane, including from the
            // batches this thread elects itself. Writes posted BEFORE this
            // thread took the staging lock are already in the lane's arrival
            // buffer, and every acquisition below folds that buffer into
            // `pending` first (M7: `lock_state`), so the idle test cannot
            // step over one. Publishing the drain fact cancels the cohort
            // clause of the M6 hybrid seal: without it, every batch elected
            // during a drain would wait out its whole capture ceiling for
            // arrivals that cannot come. (Lease and flush posts do not take
            // the staging lock and so are still possible; sealing early on
            // their account is the conservative direction.)
            lane.lock_state()?.lanes_idle_waiters += 1;
            lane.pipeline_cvar.notify_all();
            let drained = self.drain_block_journal_lane(shard, lane);
            if let Ok(mut state) = lane.lock_state() {
    ...
    fn drain_block_journal_lane(
        ...
        loop {
            // Folds in any arrival posted before the caller took the
            // staging lock: an un-absorbed post must never read as idle.
            let mut state = lane.lock_state()?;
    ...
    fn coalesce_block_journal_requests<'a>(
        ...
        loop {
            ...
            let (next_state, timed_out) = lane.wait_arrivals_timeout(state, delay)?;
            state = next_state;
    ...
    /// Drive one elected batch through the two-stage lane pipeline.
    ///
    /// The caller holds the WRITE stage and allocated `ticket` on `lane`
    /// (passed in resolved so this function is infallible before the ticket
    /// lifecycle starts). This function owns the rest of the lifecycle: it
    /// always releases the write stage, always advances the sync ticket
    /// exactly once, and always completes every request id exactly once.
    fn persist_block_journal_lane_batch(
        ...
        lane: &BlockJournalFlushCoordinator,
        ticket: u64,
        mut batch: BTreeMap<u64, BlockJournalLaneRequest>,
    ) -> Result<()> {
        if batch.is_empty() {
        ...
        lane_timing.enqueue_staging_lock_wait_nanos =
            carried_enqueue_waits.staging_lock_wait_nanos;
        lane_timing.enqueue_reserve_lock_wait_nanos =
            carried_enqueue_waits.reserve_lock_wait_nanos;
        ...
        let write_result = (|| -> Result<AppendVisibleJournalProfile> {
            ...
            #[cfg(test)]
            self.pause_block_journal_write_stage_if_requested_for_test()?;
            // The append holds the shard's journal lock only for the file
            // write; the data sync runs outside it so concurrent acknowledged
            // appends are not serialized behind this batch's durability
            // boundary. A poisoned shard tail is repaired inside the append
            // before any new frame is written.
            let profile = self
                .durable
                .append_block_journal_records_unsynced(shard, &records, measure)?;
            // Test-only: pause while still holding the write stage, so the
            // paused batch blocks later elections exactly like the serial
            // lane did (materialization and batching tests rely on pending
            // requests accumulating behind this pause).
            #[cfg(test)]
        ...
        //      arrival order equals per-device commit-seq order, so
        //      per-device publish order is unchanged from the serial lane.
        //      That chain has four links and every one of them is a
        //      structural property, not a timing one: (i) the sequence is
        //      allocated and the request POSTED inside one staging hold, and
        //      the staging lock totally orders holds, so seq order equals
        //      post order per lane; (ii) a fold takes the WHOLE arrival
        //      buffer, so a request posted before another is folded into
        //      `pending` no later than it; (iii) `pending` is keyed by the
        //      post-time request id and a batch takes ALL of `pending`, and
        //      those takes are themselves serialized (elections need the
        //      write stage, late-arrival takes happen at the batch's append
        //      window, i.e. in ticket order), so an earlier post is never in
        //      a later batch than a later post; (iv) within a batch the
        //      write records are sorted by commit seq.
        ...
        let publish_result = (|| -> Result<()> {
            ...
            // reserved sequences must publish in order per device, and a
            // record may reference segments only after both are durable.
        ...
        // A publish failure errors this batch's waiters but does not poison
    ...
    fn wait_for_block_journal_append_window<'a>(
        ...
        if state.next_sync_ticket != ticket {
            ...
            while state.next_sync_ticket != ticket {
                state = lane.wait_pipeline_arrivals(state)?;
            }
        }
        if let Some(captured) = captured_requests {
            ...
            if !ceiling.is_zero() && !state.cohort_seal_ready(ticket, captured, policy.target_requests)
            {
                ...
                loop {
                    ...
                    let (next_state, timed_out) =
                        lane.wait_pipeline_arrivals_timeout(state, remaining)?;
                    state = next_state;
    ...
    fn block_journal_cohort_seal_waiting_for_test(&self, device_id: DeviceId) -> Result<bool> {
        let (_, lane) = self.block_journal_lane(device_id)?;
        let waiting = lane.lock_state()?.cohort_seal_waiting;
        Ok(waiting)
    }
    ...
    fn complete_block_journal_lane_batch(
        &self,
        lane: &BlockJournalFlushCoordinator,
        mut state: MutexGuard<'_, BlockJournalFlushState>,
        request_ids: &[u64],
    ...
    /// Test-only pause at the head of the lane's SYNC stage, after the
    /// batch owns its ticket turn and before it issues the fsync: while one
    /// batch is held here "mid-sync", the pipeline's overlap and coverage
    /// accounting can be observed deterministically.
    #[cfg(test)]
    fn pause_block_journal_sync_if_requested_for_test(&self) -> Result<()> {
        let (lock_state, cvar) = &*self.block_journal_materialization_pause;
        let mut state = lock_state
            .lock()
            .map_err(|_| StorageError::conflict("block-journal materialization pause poisoned"))?;
        if !state.pause_in_sync_stage {
            return Ok(());
        }
        state.paused_in_sync_stage = true;
        cvar.notify_all();
        while !state.resume_sync_stage {
            state = cvar
                .wait(state)
                .map_err(|_| StorageError::conflict("block-journal materialization pause poisoned"))?;
        }
        state.pause_in_sync_stage = false;
        state.paused_in_sync_stage = false;
        state.resume_sync_stage = false;
        cvar.notify_all();
        Ok(())
    }

    /// Test-only pause inside the WRITE stage, after the lane poison check
    /// and payload recheck but before the journal append: holds a batch at
    /// the exact point where an earlier batch's sync failure can land
    /// before this batch's frames are written.
    #[cfg(test)]
    fn pause_block_journal_write_stage_if_requested_for_test(&self) -> Result<()> {
        let (lock_state, cvar) = &*self.block_journal_materialization_pause;
        let mut state = lock_state
            .lock()
            .map_err(|_| StorageError::conflict("block-journal materialization pause poisoned"))?;
        if !state.pause_in_write_stage {
            return Ok(());
        }
        state.paused_in_write_stage = true;
        cvar.notify_all();
        while !state.resume_write_stage {
            state = cvar
                .wait(state)
                .map_err(|_| StorageError::conflict("block-journal materialization pause poisoned"))?;
        }
        state.pause_in_write_stage = false;
        state.paused_in_write_stage = false;
        state.resume_write_stage = false;
        cvar.notify_all();
        Ok(())
    }

    #[cfg(test)]
    fn request_block_journal_write_stage_pause_for_test(&self) -> Result<()> {
        let (lock_state, cvar) = &*self.block_journal_materialization_pause;
        let mut state = lock_state
            .lock()
            .map_err(|_| StorageError::conflict("block-journal materialization pause poisoned"))?;
        state.pause_in_write_stage = true;
        cvar.notify_all();
        drop(state);
        Ok(())
    }
    ...
    #[cfg(test)]
    fn wait_for_block_journal_write_stage_pause_for_test(&self) -> Result<()> {
        let (lock_state, cvar) = &*self.block_journal_materialization_pause;
        let mut state = lock_state
            .lock()
            .map_err(|_| StorageError::conflict("block-journal materialization pause poisoned"))?;
        while !state.paused_in_write_stage {
            state = cvar
                .wait(state)
                .map_err(|_| StorageError::conflict("block-journal materialization pause poisoned"))?;
        }
    ...
    #[cfg(test)]
    fn resume_block_journal_write_stage_for_test(&self) -> Result<()> {
        let (lock_state, cvar) = &*self.block_journal_materialization_pause;
        let mut state = lock_state
            .lock()
            .map_err(|_| StorageError::conflict("block-journal materialization pause poisoned"))?;
    ...
    #[cfg(test)]
    fn request_block_journal_sync_stage_pause_for_test(&self) -> Result<()> {
        let (lock_state, cvar) = &*self.block_journal_materialization_pause;
        let mut state = lock_state
            .lock()
            .map_err(|_| StorageError::conflict("block-journal materialization pause poisoned"))?;
        state.pause_in_sync_stage = true;
        cvar.notify_all();
        drop(state);
        Ok(())
    }
    ...
    #[cfg(test)]
    fn wait_for_block_journal_sync_stage_pause_for_test(&self) -> Result<()> {
        let (lock_state, cvar) = &*self.block_journal_materialization_pause;
        let mut state = lock_state
            .lock()
            .map_err(|_| StorageError::conflict("block-journal materialization pause poisoned"))?;
        while !state.paused_in_sync_stage {
            state = cvar
                .wait(state)
                .map_err(|_| StorageError::conflict("block-journal materialization pause poisoned"))?;
        }
    ...
    #[cfg(test)]
    fn resume_block_journal_sync_stage_for_test(&self) -> Result<()> {
        let (lock_state, cvar) = &*self.block_journal_materialization_pause;
        let mut state = lock_state
            .lock()
            .map_err(|_| StorageError::conflict("block-journal materialization pause poisoned"))?;
        state.resume_sync_stage = true;
        cvar.notify_all();
        drop(state);
        Ok(())
    }
    ...
    fn stage_block_journal_segment_ref_entries(
        ...
        for write in collapsed {
            ...
            // across storage nodes and their data disks. Chunks are shared
            // windows into the collapsed buffer, not per-chunk copies.
            let mut chunk_offset = write.offset;
            let payload = SharedSegmentPayload::from_vec(write.bytes);
            let payload_len = payload.len();
            let mut chunk_start = 0_usize;
            while chunk_start < payload_len {
                let chunk_end = chunk_start.saturating_add(chunk_bytes).min(payload_len);
                let chunk = payload.window(chunk_start, chunk_end)?;
                ...
                let receipt = self.local.write_segment_for_intent_shared_verified(
    ...
    /// that lands between it and the post. Until M6 the reservation
    ...
    /// delete and happened-before this writer's post. M6 replaced the
    ...
    fn seal_staged_segment_refs_via_lane(
        ...
        // the stamp, and the POST (M6: no nested metadata mutex, no struct
        // builds, no validation inside it; M7: no lane mutex, no notify, no
        // allocation either — see `post_arrival`). Stamping the sequence
        // inside the hold is what keeps per-device arrival order equal to
        // commit-seq order, which the lane's ordered publish step requires.
        let measure = self.persist_profiler.is_enabled();
    ...
    fn commit_block_journal_segment_ref_ack_with_writer(
        ...
        //
        // THE GATE IS COMPLETE (M7). `lock_state` folds the lane's arrival
        // buffer in before this reads `unapplied_writes`, so a lane write
        // that was POSTED but not yet folded still counts. There is no
        // window on the other side either: a lane write posted after this
        // point cannot exist until this thread releases the staging lock,
        // and it then allocates a strictly later sequence and publishes
        // strictly later — which is exactly why the bypass keeps its hold
        // across the whole inline publish below.
        let (_, device_lane) = self.block_journal_lane(lease.device_id)?;
        let lane_ordered = device_lane.lock_state()?.has_unapplied_writes(lease.device_id);
        if lane_ordered {
            drop(enqueue_guard);
        ...
        let shard = self.durable.block_journal_shard_for_device(lease.device_id);
        let profile = self.durable.append_block_journal_records_unsynced(
            shard,
            &[BlockJournalRecord::Write(commit.clone())],
            measure,
    ...
    fn commit_block_journal_batch_with_writer(
        ...
        // post (or the acknowledged bypass, which keeps its long hold).
        let collapsed_range_count = usize_to_u64(collapsed.len());
        ...
        match durability {
            ...
            crate::api::WriteDurability::Acknowledged => {
                ...
                // is allowed. `lock_state` folds posted-but-unfolded arrivals
                // in first, so the gate sees them (M7; see the segment-ref
                // bypass for the full argument).
                let (_, device_lane) = self.block_journal_lane(lease.device_id)?;
                let lane_ordered =
                    device_lane.lock_state()?.has_unapplied_writes(lease.device_id);
                if lane_ordered {
                    let (shard, request_id) =
                        self.enqueue_block_journal_request(BlockJournalLaneRequest::Write {
                            commit,
    ...
    fn commit_sparse_block_journal_with_writer(
        ...
        // lock-free sequence allocation, the stamp, and the post.
        let mut commit = BlockJournalCommit {
            device_id: lease.device_id,
            writer_epoch: lease.writer_epoch,
            commit_seq: CommitSeq::from_raw(0),
