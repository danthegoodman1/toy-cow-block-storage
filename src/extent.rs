use crate::api::{ByteRange, FlushResult, ReadResponse, WriteDurability};
use crate::error::{Result, StorageError};
use crate::id::{
    AppendLeaseId, ClientEpoch, ExtentId, FileId, FileVersion, LogicalDeadline, RequestId,
    WriterEpoch,
};
...
/// preserves file-level intent such as append leases and writer epochs.
///
/// Minimal implementor guarantees:
///
/// - Successful appends are atomic file-version transitions.
/// - Stale append leases and stale writer epochs fail without exposing partial
///   file contents.
/// - Reads observe the latest committed file root/version.
...
pub trait NativeFile: Send + Sync {
    /// Return the stable ID of this file handle.
    ///
    ...
    ///
    /// Implementors must fill the whole buffer or return an error. A
    /// zero-length buffer is a no-op. Reads past the committed file size must
    /// fail rather than synthesize data.
    fn read_at(&self, offset: u64, buf: &mut [u8]) -> Result<()>;

    /// Acquire an append lease for this file.
    ///
    /// A lease carries the file version and writer epoch needed to fence stale
    /// append commits. Acquiring a new lease may invalidate older leases when
    /// the implementation supports lock stealing.
    fn acquire_append(&self) -> Result<AppendLease>;

    /// Append bytes using a previously acquired lease.
    ///
    /// Success means the append is durable and committed as one file-version
    /// transition. Stale leases must fail without exposing partial file data. A
...
/// Public native file control surface.
///
/// Implementors create/open native files without exposing root layout, segment
/// placement, or provider topology.
pub trait NativeFileClient: Send + Sync {
    /// Create a native file.
    ///
    /// Success means the initial empty file root is committed and subsequent
    /// info or read calls can observe the file.
    fn create_file(&self, request: CreateFileRequest) -> Result<FileId>;

    /// Return committed information for a native file.
    ///
    /// The returned information must come from the latest committed file head.
...
impl NativeRequest {
    ...
    pub fn validate_for_new_file(&self) -> Result<()> {
        match self {
            ...
            | Self::AcquireAppend { .. }
            | Self::Append { .. }
            | Self::Flush { .. } => Err(StorageError::invalid_argument(
...
pub struct NativeRequestEnvelope {
    ...
    /// Public native file operation being requested.
...
/// Actor boundary for native file requests.
///
/// `NativeServer` is a request coordinator for native file semantics. Future
/// storage replication should be coordinated below this API and above
...
/// - Fence conflicting append writers with file versions and writer epochs.
...
/// Transport boundary for native file requests.
///
/// Minimal implementor guarantees:
///
/// - The transport may be local or remote, but it must not change native file
///   semantics.
...
mod tests {
    ...
    fn lease(file_id: FileId) -> AppendLease {
        AppendLease {
            file_id,
    ...
    #[test]
    fn native_requests_expose_operation_and_target_file() {
        let file_id = FileId::from_raw(7);
        let request = NativeRequest::AcquireAppend { file_id };
