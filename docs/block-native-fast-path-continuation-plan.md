...
| Phase 4 | Direct-I/O error-kind tally at 64k/256k c16/c32 | ~~4A errno plumbing~~ **ready** (`08eabdb`) | Decides whether Phase 4 is a root-cause phase or a deletion. Run the direct-I/O cells with `--matrix-csv` set (the sidecar derives from it; without the flag, errors report only on stderr). |
...
| Complete | Work | M6: staging critical section collapse + cohort capture (re-entry of the 3D-c deferral, plus the 3A seal-point regression) | Landed 2026-07-24 after the M5 gate trip. PART (a): `next_commit_seq` hoisted out of the global metadata mutex into an `AtomicU64` on `InMemoryMetadataPlane` (`MetadataInner::next_commit_seq` demoted to a snapshot mirror refreshed at snapshot points); the three lane-enqueue holds collapse to `{deletion fence, fetch_add, stamp, enqueue}` with commit build and `validate` moved out, while the TWO ack-bypass arms keep their full holds (a lane write slipping between the bypass's lane_ordered check and its inline publish would make the bypass publish first and the lane's earlier seq hit the stale-commit-seq conflict = whole-batch error in steady state). The reservation's `device_heads` probe, which was the fence keeping deleted-device frames out of the journal, is replaced by a `device_delete_generation` atomic read BEFORE the `device_info` witness and re-read inside the staging hold — the ordering is load-bearing (the delete's bump shares the metadata-mutex hold the witness takes, so a removal ordered after the witness is always detected; reading it after the witness would be silently wrong). Without this the enqueued frame for a deleted device becomes durable and bricks `open()` permanently at replay. Two allocator bugs inherited from the milestone's first (terminated) implementer were found via worktree bisect and fixed: materialization advanced only the mirror so the live allocator re-issued consumed sequences (27 failing tests), and the prepared-publish gap check read the stale mirror. PART (b): hybrid seal — the append window now also requires `captured + pending >= target_requests` OR a requested drain OR (publish edge passed AND the wave woke AND re-entered), with the cohort phase alone bounded by the lane's own last successful fsync capped at `max_coalesce_delay`, a one-bit predictor disarming the wave clause after any ceiling-bound wait, and `enqueue` notifying `pipeline_cvar` under a parked-owner flag. Batches capturing no write are non-capturing by construction (a flush/lease-only batch elected inside a root-copy section holds the global staging lock, so a capture wait there would stall every writer). Local A/B vs cee1cc6 (`.abench/results/phase3-m6-final-ab-20260724`, fixed harness with md5 manifest, run on the post-F1 binary; corroborated by `phase3-m6-coordinator-ab-20260724` at 5 reps on the flagged cells): block-write-4k-shard-lanes c4 2.14x (p99 0.53), block-write-4k-device-lanes c16 1.99x (p99 0.55), device-lanes c32 1.35x, shard-lanes c1 1.14, batch-4k-16ops c1 1.08 / c16 1.03 / c32 1.01, all writeback and read cells >= 0.98 including the historically bimodal block-read-1m c32 at 0.998. ONE EXPLAINED REGRESSION, recorded rather than tuned away: block-batch-4k-256ops c4 at 0.944 (base 2880-2945 vs fix 2765-2785 MBps, non-overlapping across four reps per side in two independent runs, so not sampling noise). Mechanism from the profile split: cohort capture engages (grp 1.03 -> 1.15, batches 13,565 -> 11,506) but `pipeline_order_wait` rises 32-35 -> 93-95 us/batch, because a 256-write resubmission on the segment-ref path carries a data-log fsync and the wave is slow to return, so the wait outruns the coalescing gain (net ops 13,972 -> 13,232 = -5.3%, matching the throughput delta). This is a local guard cell, not a gate cell (the c4 gate cell is block-batch-4k-16ops, which is parity-to-positive), and it sits at the boundary of the standing 5-10% explained-regression band. Deliberately NOT tuned before the confirm trip: the trip is the arbiter for whether cohort capture pays at all on real hardware, and tuning a ceiling policy against null_blk ahead of that evidence is the mistake this phase has twice avoided. If the trip shows capture paying, this is an accepted trade; if not, part (b)'s ceiling policy is reopened with GCP data. grp distributions (the pre-registered bar): shard-lanes c4 1.91 -> 3.97 with solo batches 31.8% -> 0.1%; device-lanes c16 1.58 -> 3.17 with solo 50.0% -> 0.1%; deep regime untouched (shard-lanes c16/c32 7.73/15.45 -> 7.75/15.45). Cohort ceiling binds on 0.50% of waits (131,072 sampled). HONEST NEGATIVE, pre-registered: part (a)'s `enqueue_staging` bucket does NOT collapse locally (the local waits are intrinsic same-device ordering per the M4 finding, not the GCP preemption convoy) — the confirm trip is the arbiter for part (a). Skeptical review: design pass (which overturned a coordinator ruling on the delete race), full pass verifying all eight construction claims from code with a complete wake-edge audit, and a focused-fix pass, explicit APPROVED. |
| Incomplete | Finding | OUT OF SCOPE — two remaining survivor paths for journal records naming a deleted device | Found 2026-07-24 by the reviewer of the lease-brick fix, which closes the main path but not these. (a) Prune skips poisoned shards (`src/local/durable/sqlite.rs:3784`), and the poison clears only when the lane next appends to that shard (`recover_block_journal_tail_locked`, `:3457`) — which may never happen if the deleted device was that shard's only user. (b) `acquire_block_writer` (`src/local/durable/coordinator.rs:4032`) has NO delete fence, unlike the write path's `ensure_block_journal_device_live_in_staging_hold` (`:3952`), so a `Lease` record can be appended for a device whose delete has already pruned. Both are benign while delete evidence stands (replay skips them) and become a brick — or, after a `fork_device` into the freed id, silent cross-device data application — once `run_metadata_custodian` expires the evidence. Reviewer built the silent-corruption case and confirmed it reproduces IDENTICALLY with the lease fix stubbed out, so it is pre-existing and not a regression. Also observed: with evidence expired and a record stranded, `run_metadata_custodian` itself fails with `NotFound { kind: "device" }` from the materialization scan — loud, but the store loses its custodian. |
| Incomplete | Finding | OUT OF SCOPE — pre-existing: the block-journal overlay is not cleared on `delete_device` | Found 2026-07-24 in passing by the same reviewer. After a `fork_device` re-takes an id freed by retention expiry, IN-PROCESS reads of the reborn device return the old device's bytes from the overlay (reviewer read `99`s from a device forked from one holding `71`s). Reopen hides it because the overlay is rebuilt from the journal. Confirmed identical with the lease fix stubbed out, so pre-existing and unrelated. Deserves its own change: `delete_device` should drop the deleted device's overlay state. |
| Incomplete | Finding | OUT OF SCOPE — pre-existing latent data-durability defect in `persist_physical`: bounded persists can prune acknowledged Flushed writes | Found 2026-07-24 while fixing the lease brick; REPRODUCED as real loss by the reviewer, in a disposable tree. Two devices written with `WriteDurability::Flushed`, then `persist_until(1)`: `target=Some(CommitSeq(1))`, `image_shard_commits_max=Some(1)`, `live_materialized=[(1,1),(2,2)]`; device B's journal record classes go `{flush,lease,write}` -> `{lease}`, and after a crash-reopen A reads back correctly while **B's block 0 reads zeros — acknowledged Flushed data silently lost**. Cause is entirely pre-existing and separate from the lease fix (the fix's `retired` set is empty throughout, so its code is provably inert here): `block_journal_commit_is_materialized` is judged against a LIVE high-water that can sit past the bounded image's truncation point, so records are pruned for commits the image does not carry. SEVERITY (corrected 2026-07-24): REACHABLE THROUGH THE PUBLIC API — HIGH. `flush_file` is `pub` and persists only to that file's own `latest_commit`, so flushing an unrelated file while `Flushed` block writes are in flight produces a bounded persist as a side effect and loses them. The earlier "no public trigger" assessment was sound for the probe it used and wrong in general: a single block write followed by `flush_file` PASSES, because the native-delta fast path returns before the prune; reaching the loss needs a second block write — one before the file commit to put a pending delta in the exported range and defeat that fast path, one after to be the victim. Regression test `durable_flush_file_does_not_prune_concurrent_block_writes_past_its_commit` uses only public methods. It is masked by three unrelated mechanisms, none of which is a designed durability invariant: `requested_through` is a monotone max that every unbounded persist bumps to `durable_through` (and create/fork/delete/custodian all persist unbounded); `state_for_durable_persist_through` falls back to unbounded when `target >= current_commit` (`src/local/durable/coordinator.rs:173-175`); and the native-delta fast path (`:2369-2405`) returns before the prune. FIX (symmetric to what the lease change did for `retired`): derive the materialized high-water from the image — `block_materialized_high_water_locked(&image.metadata)` at `src/local/durable/coordinator.rs:2444`. Deliberately not fixed alongside the lease work: real blast radius on the M4 prune tests. |
| Incomplete | Finding | Follow-ups from the lease-brick fix review (non-blocking) | N1 (coverage): swapping the prune's `retired` source from the persisted image back to a live read leaves ALL 430 tests passing — the most safety-critical decision in that change has zero test discrimination. A pinning test is blocked on the same private `persist_until` access the data-loss repro above needed, so it lands naturally with that fix. N2 (doc precision): `retired_block_device_ids_locked`'s claim that the subtraction removes all dependence on `deleted_device_heads` and `delete_records` staying coupled is unconditional at the two live-view sites but not at prune — on a bounded persist `image.metadata.device_heads` means "live as of target", so a device created after target is absent from the image entirely and a stale `DeleteRecord` with seq <= target naming it would survive the subtraction. Needs the two artifacts to decouple first, which they cannot today (`src/local/metadata_plane.rs:2919-2925`; `src/local/durable/persist.rs:13-15/35-41/113-119`). One sentence of scoping, no residual-risk change. |
| Complete | Work | M7: remove the lane enqueue from the staging hold (final Phase 3 milestone) | Landed 2026-07-24. MEASURED FIRST, which redirected the fix: instrumenting the post-M6 staging hold on the inline flushed path at 4k c32 (n=327,680 holds) decomposed it as notify_all 3.403us (61.0%), lane.inner unlock/futex-wake 0.997us (17.9%), contended lane.inner acquire 0.784us (14.0%), pending insert 0.140us (2.5%), and deletion fence + alloc_commit_seq + stamp 0.064us (1.15%) of a 5.578us total. The nested enqueue is 95.4% of the hold and everything M6 shortened is 1.15% — which predicts M6's 7.8% result almost exactly. The refinement that mattered: the dominant term is WAKEUP WORK (notify + unlock wake = 79%), not queueing on lane.inner (14%), so the coordinator's stated hypothesis was right about the site and wrong about the cause. FIX: each lane gains an `arrivals: Mutex<Vec<(u64, request)>>` leaf buffer plus an AtomicU64 id source; `enqueue_block_journal_request` becomes `post_arrival` (fetch_add + one Vec::push under the leaf lock — no lane mutex, no notify, no allocation after warm-up, since the drainer returns its emptied buffer), and every `lane.inner` acquisition and condvar wake folds the buffer into `pending` first via `lock_state`/`wait_arrivals`/`wait_pipeline_arrivals`. Net per op: one fewer lane-mutex acquisition and one fewer unlock wake, not a relocation. Owner-allocated sequences (the coordinator's preferred direction) was evaluated and REJECTED with a structural argument: the post must stay inside the staging hold regardless, because that hold is simultaneously the deletion fence and the ack-bypass barrier, so it would remove only the 0.064us fetch_add — and pairing it with post-outside-staging breaks the bypass, letting the owner allocate a smaller seq for a write that entered after the bypass's gate and publish it later (the stale-seq inversion). std::sync::mpsc rejected because atomic whole-buffer take is exactly what the ordering argument needs and a receive-until-empty loop cannot supply it in one step. Local A/B vs 1d89c87 (`.abench/results/phase3-m7-coordinator-ab-20260724`, fixed harness, md5s recorded, 2 reps + a 5-rep tiebreak on flagged cells): the target column `block_journal_enqueue_staging_lock_wait_nanos` collapses 33.98 -> 0.013 us/op at shard-lanes c32 (-99.96%) with grp intact (15.45 -> 15.56); block-write-4k-device-lanes c32 1.220 / c16 1.030 / c4 1.057 / c1 1.075 with p99 0.71-0.84; block-batch-4k-16ops (the locally trustworthy cell) c16 1.056 / c32 1.032; block-batch-4k-256ops c4 1.127 (p99 0.598); shard-lanes c1 1.214 (p99 0.555); reads parity. TRACKED LOCAL REGRESSION, explained: block-write-4k-shard-lanes c16 0.92 on 5-rep medians (base 117-125, fix 107-121, ranges barely overlapping, so not sampling noise) — but p99 IMPROVES to 0.78, grp is UP (7.73 -> 7.84) so cohort capture is unaffected, and the profile split attributes it to the sync bucket (+2.3%) with 5% fewer batches: the documented null_blk artifact where a CPU-bound fsync absorbs freed writer time, the same signature as M3's shard-lanes c32 0.977 which the GCP trip then showed as fine. The same workload is at parity at c32 (0.993). block-writeback-fsync-1m c1 and block-read-1m c32 were flagged at 2 reps and resolved as noise at 5. Skeptical review: full pass verifying all six construction claims from code (including an exhaustive sweep proving `lane.inner` has exactly one production acquisition site and all three `pending` takes are whole-map and serialized), then two BLOCKING test defects found and fixed — the new stress test HUNG instead of failing when it detected the inversion it exists to detect (measured 80 runs under mutation: 0 failures, 5 hangs; after a Drop guard, 4 failures, 0 hangs), and a new sparse assertion was vacuous (asserted zeros on a device that reads zeros anyway; now pre-fills 0xAB and is mutation-verified). Explicit APPROVED after re-verifying the production path byte-identical. |
| Incomplete | Finding | TOOLING — `cargo fmt --check` covers almost none of the crate | Found 2026-07-24 by the M7 reviewer. `src/local/mod.rs` `include!`s the production files (`coordinator.rs`, `durable/coordinator.rs`, `durable/block_journal.rs`, `durable/sqlite.rs`, `metadata_plane.rs`, and the rest at lines 66-91) rather than declaring them with `mod`. rustfmt only formats files reachable through `mod`, so the `cargo fmt --check` step every milestone in this phase ran as part of its gate was examining essentially only `src/local/tests/mod.rs` and the handful of genuinely `mod`-declared files. Verified by the coordinator: running `rustfmt --check` directly on four of the include!d files reports **zero** diff hunks, so no drift has actually accumulated (editors and manual runs have kept them clean) and nothing needs reformatting — but the gate has been asserting something it does not check, for the whole phase. Fix options: declare the modules properly (large mechanical change, and the single-flat-module layout is load-bearing for the crate's cross-file privacy, see the M1 codegen-unit finding), or add an explicit `rustfmt --check` pass over the include!d files to the gate. Recorded rather than fixed because it is repo-wide tooling, not Phase 3 work. |
| Incomplete | Gate | M7 pre-registered stop clause (written 2026-07-24, BEFORE the work) | The confirm trip (`phase3-confirm-20260724`) refuted M6 part (a): collapsing the staging hold to `{fence, fetch_add, stamp, enqueue}` moved the 4k c32 convoy only 289.3 -> 266.8 us/op (-7.8%), so hold LENGTH is not the cause. Remaining hypothesis, code-visible: the global staging lock is still held ACROSS `enqueue_block_journal_request`, which acquires the contended `lane.inner` mutex and issues `notify_all` — writers queue on staging while the holder queues on a second lock the lane owner holds during election and seal. M7 gets ONE attempt at removing that nesting. PRE-REGISTERED STOP, binding: if the post-M7 confirm trip shows fix-side `block_journal_enqueue_staging_lock_wait_nanos` per op at 4k c32 NOT reduced by at least 50% (i.e. not <= ~133 us/op), OR 4k c32 throughput below 200 MBps, then Phase 3 CLOSES with the gate unmet and the convoy handed forward as a documented residual — no M8, no further attempts under a Phase 3 flag. Rationale for the bar: the closed-loop arithmetic that projects the 220.73 MBps gate requires most of the ~267 us/op to disappear; a partial win that leaves the convoy dominant does not reach the gate and would only buy another speculative milestone. This phase has already refuted three plan premises by measurement (V1 lane pipelining, M4 stripes, M6 hold-shortening), which is the evidence that speculative continuation is the failure mode to guard against. |
| Incomplete | Finding | OUT OF PHASE 3 SCOPE — pre-existing HIGH-severity bug: a leased-then-deleted device bricks `open()` permanently | Found during M6 by the implementer, then confirmed end to end by an independent reviewer against cee1cc6 (M6 touches neither prune nor replay, so it is unchanged by Phase 3 and predates it). Chain: `acquire_block_writer` enqueues a `Lease` request and the lane writes `BlockJournalRecord::Lease` (`src/local/durable/coordinator.rs:4487`); `block_journal_record_pruned` (`src/local/durable/sqlite.rs:2468`) matches `Lease { .. } => out.push(record)` UNCONDITIONALLY, so no prune path ever removes it; `delete_device` (`src/local/metadata_plane.rs:3840`) removes the head without touching `shard_commits`, so the device's Write/Flush records prune correctly and the Lease record is the sole survivor; at reopen `load_block_journal_overlay` (`src/local/durable/block_journal.rs:2801`) -> `seed_block_writer_epoch` -> `metadata.device_info(device_id)` -> NotFound, propagated with `?` from `DurableCoordinator::open` (`src/local/durable/coordinator.rs:957`). No recovery path, no tolerance: `open()` fails store-wide and permanently. Requires a prior lease (only `acquire_block_writer` emits Lease records); NOT cleared by materialization or prune. Repro needs no lane write at all: create device -> `acquire_block_writer` -> `delete_device` -> reopen fails. Latent only because the one existing delete-then-reopen test (`src/local/tests/mod.rs:15774`) writes via `write_device` without taking a lease. The lease API is the block fast path, so the exposure is a normal workflow. Fix belongs in its own change (replay must tolerate — or prune must drop — Lease records for devices absent from restored metadata, distinguishing delete evidence from unknown ids so corruption detection is preserved); deliberately NOT folded into the Phase 3 gate chase. |
| Complete | Gate | Targeted GCP 4k run meeting the completion gate | RESOLVED 2026-07-25: **one gate condition MET robustly, the other a statistical tie**. Final trip `phase3-m7-gate5x-20260725` (base a7c14a2 = Phase 2 close vs fix 36faff7 = Phase 3 through M7, same instance, interleaved, 5 reps, both sides pinned-profile, ZERO errors across every cell and rep, teardown verified). **4k c32 throughput: MET** — fix 231.43 MBps mean against the 220.73 bar (1.1x Ceph's 200.66), and robustly so: all five reps clear it individually (223.0 / 232.3 / 234.7 / 236.6 / 230.6) against a base of 166.52, i.e. **1.390x over Phase 2's close**. **4k c4 p99: NOT DEMONSTRATED** — fix 591.85us mean against the 585.7us bar. It improved on base (620.37us, -4.6%) but the bar sits 0.27 standard errors from the mean (sd 51.5, stderr 23.1 over 5 reps: 658.5 / 623.4 / 555.1 / 592.3 / 529.9), so this cell is statistically indistinguishable from the Ceph reference and the claim cannot be made. NOTE the methodology lesson: a 2-rep trip (`phase3-m7-confirm-20260724`) reported this cell at 576.3us and would have been recorded as MET; five reps moved it to 591.85us. Both conditions straddled their bars per-rep at 2 reps, which is why the trip was repeated before the gate was scored — the earlier PASS was sampling luck. Remaining gate language: c1 and c16 are within tolerance (throughput 1.125x and 1.028x), with one tracked regression — 4k c16 p99 +10.5% (650.75 -> 719.02us). ATTEMPT 1 (`phase3-gate-20260723`, base a7c14a2 vs cee1cc6): gate UNMET at 4k c32 165.6 MBps with a 4k c4 regression to 0.83x; the M1 columns named the blocker as 289us/op of staging-lock convoy under GCP preemption on a single-device workload, which M6 and M7 then attacked. |
...
| Status | Type | Item | Evidence / Gap |
| --- | --- | --- | --- |
| Complete | Work | 4A: loadbench error-kind tallies with errno recovered per cell | Commit `08eabdb`. **`StorageError` unchanged** — the plan's premise that the errno was destroyed by stringification is wrong: `src/local/util.rs:89` `fs_error` is a single funnel and `io::Error`'s Display already carries `(os error N)`; the loss was entirely in loadbench (`worker.rs` passed `result.is_ok()`). Verified end to end — a real EACCES injected mid-run tallies 22,041 errors inside the measured loop, errno intact, on both backends. `matrix.csv` gains 8 appended numeric columns (6 per-variant counts, top errno, top-errno count); errno and verbatim first-error samples go to a `matrix.errors.csv` sidecar derived from `--matrix-csv`, collected by the existing whole-dir tarball with no infra change. Grouping key `(kind, overflow, errno, digit-normalized prefix)`; the prefix is what keeps the three errno-less direct-I/O write failures from collapsing into one bucket. Tests mutation-verified (4 mutants by the implementer, re-run plus 3 more by the reviewer). |
| Incomplete | Work | 4B: local O_DIRECT reproduction | Missing: repro recipe and observed error kind/errno. **Premise partly refuted while landing 4A**: the scope text says the container's overlay filesystem will not exercise `O_DIRECT`, so a loop-mounted XFS image is needed. In fact `O_DIRECT` opens succeed on this box's tmpfs *and* ext4, so the local backend is reachable without the loop-mount ceremony. Whether a loop device reproduces NVMe *queue-pressure* errno behaviour is still open and still non-exonerating — but the setup cost of starting 4B is lower than recorded. |
| Finding | Scope | The historical direct-I/O error counts are an upper bound, not a syscall count | `coordinator.rs` replays a stored `last_error` to every cohort member waiting on a generation (sites at 160, 325-329, 1884-1909, 2038-2063, 2202-2278, 2306-2311, 2516-2544, 2767-2792, 3127-3148, 3330-3360; each clones and returns verbatim). One physical I/O failure can therefore be counted up to 32 times at c32. The `stage6-direct-io-*` figures (64k c32 238,573; 256k c32 409,418) are op-level failures and may reflect ~32x fewer distinct syscall failures. The 4A tally inherits the same property — `count` is op-level. Do not read the trip as "409k syscalls returned EAGAIN". |
| Finding | Scope | `StorageError::Unsupported` from the direct-I/O backend is unreachable on Linux | `open_direct_io_file` and `read_exact_at_direct` have `#[cfg(not(target_os = "linux"))]` `Unsupported` arms (`io_backend.rs:508-513`, `:522-527`), so on any Linux host — dev container or GCP — an `O_DIRECT` rejection surfaces as `open()` EINVAL through `fs_error`, i.e. `Unavailable "(os error 22)"`. Any future test or diagnostic that expects `Unsupported` to signal "this filesystem refused O_DIRECT" is testing a path that cannot execute where it matters. |
