...
| Incomplete | Finding | OUT OF SCOPE — pre-existing latent data-durability defect in `persist_physical`: bounded persists can prune acknowledged Flushed writes | Found 2026-07-24 while fixing the lease brick; REPRODUCED as real loss by the reviewer, in a disposable tree. Two devices written with `WriteDurability::Flushed`, then `persist_until(1)`: `target=Some(CommitSeq(1))`, `image_shard_commits_max=Some(1)`, `live_materialized=[(1,1),(2,2)]`; device B's journal record classes go `{flush,lease,write}` -> `{lease}`, and after a crash-reopen A reads back correctly while **B's block 0 reads zeros — acknowledged Flushed data silently lost**. Cause is entirely pre-existing and separate from the lease fix (the fix's `retired` set is empty throughout, so its code is provably inert here): `block_journal_commit_is_materialized` is judged against a LIVE high-water that can sit past the bounded image's truncation point, so records are pruned for commits the image does not carry. SEVERITY: MEDIUM as it stands, because the reviewer could not trigger it through any public API, but the failure mode is HIGH (silent loss of acknowledged Flushed writes on crash). It is masked by three unrelated mechanisms, none of which is a designed durability invariant: `requested_through` is a monotone max that every unbounded persist bumps to `durable_through` (and create/fork/delete/custodian all persist unbounded); `state_for_durable_persist_through` falls back to unbounded when `target >= current_commit` (`src/local/durable/coordinator.rs:173-175`); and the native-delta fast path (`:2369-2405`) returns before the prune. FIX (symmetric to what the lease change did for `retired`): derive the materialized high-water from the image — `block_materialized_high_water_locked(&image.metadata)` at `src/local/durable/coordinator.rs:2444`. Deliberately not fixed alongside the lease work: real blast radius on the M4 prune tests. |
