...
| Status | Type | Item | Evidence / Gap |
| --- | --- | --- | --- |
| Incomplete | Work | 2A: publish-time profile split (routing vs evidence vs verify vs dispatch) | Missing: profile columns or a measurement note deciding which cost dominates. |
| Incomplete | Work | 2B: direct segment-to-node routing in receipt lookup and mark dispatch, grouped by carried node id | Missing: implementation in `src/local/storage_node.rs` and `src/local/durable/block_journal.rs` publish path. |
| Incomplete | Work | 2C: per-node-per-batch `MarkReferenced` transport and evidence-cost reduction, with contract decision note | Missing: decision note on what the custody contract requires per segment vs per batch, plus implementation. |
| Incomplete | Work | 2D: duplicate-catalog invariant relocated to a background/materialization sweep | Missing: sweep implementation and test. |
| Incomplete | Test | Routing, wrong-node, failure-injection, and replay tests | Missing: test names in `src/local/tests/mod.rs`. |
| Incomplete | Gate | Local A/B minimum gate with no >5-10% regression | Missing: before/after loadbench artifacts. |
