...
| Incomplete | Gate | Targeted GCP 4k run meeting the completion gate | ATTEMPT 1 (`phase3-gate-20260723`, base a7c14a2 vs fix cee1cc6, same instance, interleaved, both sides pinned-profile): GATE UNMET. 4k c32 165.5/165.6 MBps fix vs 168.7/167.7 base, against the 220.7 target (1.1x Ceph's 200.66); 4k c4 REGRESSED to 0.83x (57.3/58.0 vs 70.7/67.9) with grp 2.5 -> 2.1; c16 throughput flat with p99 +25%; 64k +0.1%/+1.2% (c16/c32) confirming the lane is now sync-bound at ~112us as the Phase 2 handoff predicted; reads IMPROVED (block-read-4k c16 +9.5%) so the overlay watch item did not fire. DIAGNOSIS, named by the M1 columns on the fix side: `block_journal_enqueue_staging_lock_wait_nanos` = 2.77ms/batch ~= 289us/op at 4k c32 (vs ~30us/op locally) — the global staging lock convoy under GCP preemption (32 vCPUs, 32 workers, holder preempted inside the hold; the same amplification class as the Phase 2 catalog convoy, and it does not reproduce on the local box). Mechanism fact: block-batch workloads run ONE device (`src/bin/loadbench/runner.rs:144-148`), so this is same-device serialization and lock sharding cannot help (M4 proved this) — the lever is shortening the hold, which is what M6 does. Closed-loop projection: removing ~289us/op from ~790us/op latency yields ~64k ops/s ~= 262 MBps > gate. The c4 regression is the 3A seal point forfeiting the completion resubmission wave, also addressed in M6. Missing: confirm trip with the M6 tree. |
