...
gcloud compute ssh "${VM_NAME}" \
  --project "${PROJECT}" --zone "${ZONE}" \
  --command "sudo env MIN_LOCAL_SSDS='${MIN_LOCAL_SSDS}' STORAGE_NODES='${STORAGE_NODES}' CONCURRENCY='${CONCURRENCY}' IO_SIZES='${IO_SIZES}' TOY_RTTS='${TOY_RTTS}' TOY_DURABILITIES='${TOY_DURABILITIES}' TOY_REPEATS='${TOY_REPEATS}' SKIP_TOY='${SKIP_TOY}' DELAY_MODE='${DELAY_MODE}' DURATION_MS='${DURATION_MS}' WARMUP_MS='${WARMUP_MS}' FIO_RUNTIME_SEC='${FIO_RUNTIME_SEC}' FIO_RAMP_SEC='${FIO_RAMP_SEC}' CEPH_POOL_SIZE='${CEPH_POOL_SIZE}' RBD_SIZE_MIB='${RBD_SIZE_MIB}' SOURCE_REF='${SOURCE_REF}' bash /tmp/remote_block_vs_rbd.sh '${RUN_ID}' /tmp/source.tgz"
run_status=$?
set -e
