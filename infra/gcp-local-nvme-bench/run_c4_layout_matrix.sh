...
gcloud compute ssh "${VM_NAME}" \
  --project "${PROJECT}" --zone "${ZONE}" \
    --command "sudo MIN_LOCAL_SSDS='${MIN_LOCAL_SSDS}' STORAGE_NODES='${STORAGE_NODES}' CONCURRENCY='${CONCURRENCY}' WORKLOADS='${WORKLOADS}' APPEND_PUBLISH_BATCH_TARGET='${APPEND_PUBLISH_BATCH_TARGET}' APPEND_PUBLISH_IDLE_COALESCE_US='${APPEND_PUBLISH_IDLE_COALESCE_US}' APPEND_PUBLISH_MAX_COALESCE_US='${APPEND_PUBLISH_MAX_COALESCE_US}' APPEND_INGEST_MAX_IN_FLIGHT_MIBS='${APPEND_INGEST_MAX_IN_FLIGHT_MIBS}' APPEND_INGEST_ACTIVE_LOG_LANES='${APPEND_INGEST_ACTIVE_LOG_LANES}' STREAM_AUTO_PERSIST_MIBS='${STREAM_AUTO_PERSIST_MIBS}' LAYOUTS='${LAYOUTS}' NODE_RAID_GROUPS='${NODE_RAID_GROUPS}' RTTS='${RTTS}' REPEATS='${REPEATS}' RANDOMIZE_RTT_ORDER='${RANDOMIZE_RTT_ORDER}' DELAY_MODE='${DELAY_MODE}' WARMUP_BEFORE_MEASURED='${WARMUP_BEFORE_MEASURED}' WARMUP_RTT_US='${WARMUP_RTT_US}' WARMUP_CONCURRENCY='${WARMUP_CONCURRENCY}' bash /tmp/remote_c4_layout_matrix.sh '${RUN_ID}' /tmp/source.tgz"
run_status=$?
set -e
