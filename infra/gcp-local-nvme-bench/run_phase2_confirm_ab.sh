...
gcloud compute ssh "${VM_NAME}" \
  --project "${PROJECT}" --zone "${ZONE}" \
  --command "sudo env MIN_LOCAL_SSDS='${MIN_LOCAL_SSDS}' STORAGE_NODES='${STORAGE_NODES}' AB_REPEATS='${AB_REPEATS}' WRITE_CONCURRENCY='${WRITE_CONCURRENCY}' READ_CONCURRENCY='${READ_CONCURRENCY}' GUARD_CONCURRENCY='${GUARD_CONCURRENCY}' TOY_DURABLE_IO_BACKEND='${TOY_DURABLE_IO_BACKEND}' TOY_CHUNK_MIB='${TOY_CHUNK_MIB}' DELAY_MODE='${DELAY_MODE}' DURATION_MS='${DURATION_MS}' WARMUP_MS='${WARMUP_MS}' DEVICE_BLOCKS='${DEVICE_BLOCKS}' SHARDS='${SHARDS}' BASE_REF='${BASE_REF}' FIX_REF='${FIX_REF}' bash /tmp/remote_phase2_confirm_ab.sh '${RUN_ID}' /tmp/base-source.tgz /tmp/fix-source.tgz"
run_status=$?
set -e
